...
//		assertEquals("", serverName + "  [Started, Synchronized]", server.getText());
	...
	public static boolean isProjectDeployed(String projectName) {
		...
		// Status is not checked due to JBIDE-11928
		ServersView serversView = new ServersView();
		SWTBotTreeItem server = serversView.findServerByName(serverName);
		server.expand();
		
		SWTBotTreeItem[] project = server.getItems();
		for (int i = 0; i < project.length; i++) {
			System.out.println(project[i].getText());
			if(project[i].getText().startsWith(projectName)) {
				return true;
			}
		}
		return false;
